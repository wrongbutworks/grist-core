...
describe("CalendarView", function() {
  ...
  afterEach(async function() {
    // Keep tests independent: a mistimed grid drag can leave TUI's form popup open, and its
    // overlay would intercept clicks in the next test.
  ...
  it("autofocuses the title when the form popup opens, and doesn't steal focus after", async function() {
    // Drag on an empty grid to open TUI's create-event form popup (the same path a user takes to
    // add an event by selecting a time range).
    await gotoCleanDay(9);
    ...
    await driver.withActions(a => a
      ...
      .pause(120)
      .release());
    // The popup opens and its title input receives focus (so the user can type the name right away).
    await driver.findWait(".toastui-calendar-popup-container", 2000);
    await driver.wait(async () => await activeFieldName() === "title", 2000);

    // Clicking into another field (the start-date input) must keep focus there: the popup-focus
    // observer only fires on the closed->open transition, so a later mutation won't yank focus back
    // to the title. We move focus in-page and then force a mutation inside the popup.
    await driver.executeScript(`
      const popup = document.querySelector(".toastui-calendar-popup-container");
      popup.querySelector("input[name='start']").focus();
      const m = document.createElement("span"); popup.appendChild(m); m.remove();
    `);
    // Give the observer a tick; focus must still be on the start field, not stolen back to title.
    await driver.sleep(100);
    assert.equal(await activeFieldName(), "start");
  });

  it("lists records with a title but no start date in the unassigned panel", async function() {
    await gu.sendActions([
      ["AddRecord", "Table1", -1, { From: null, To: null, Label: "No Date Yet", IsFullDay: false }],
    ]);
    await driver.wait(async () => (await getUnassignedTitles()).includes("No Date Yet"), 2000);
    // The panel and a chip for it are rendered...
    assert.isTrue(await driver.findContentWait(".test-calendar-unassigned-chip", /No Date Yet/, 2000).isDisplayed());
    // ...and the row is NOT an event on the grid.
    const rowId = await rowIdByLabel("No Date Yet");
    assert.isNull(await getCalendarEvent(rowId));
  });

  it("selects the grid row when an unassigned chip is clicked", async function() {
    const rowId = await rowIdByLabel("No Date Yet");
    const chip = await driver.findContentWait(".test-calendar-unassigned-chip", /No Date Yet/, 2000);
    await chip.click();
    // The calendar tracks the clicked row as its selected record (via the linked cursor), even
    // though a date-less row isn't drawn on the grid.
    await driver.wait(async () => (await getSelectedRecordId()) === rowId, 2000);
  });

  it("assigns a date when an unassigned chip is dragged onto the grid", async function() {
    // Day view on a clean day gives a full-width grid, so the drop lands on a known date.
    const day = await gotoCleanDay(7);
    const chip = await driver.findContentWait(".test-calendar-unassigned-chip", /No Date Yet/, 2000);
    const grid = await driver.find(".test-calendar-widget");
    // Press, jiggle past the drag threshold, move onto the grid, release. Manual pointer events
    // (the chip uses mousedown-based dragging, not HTML5 drag, which WebDriver can't drive).
    await driver.withActions(a => a
      .move({ origin: chip })
      .press()
      .move({ origin: chip, x: 0, y: 10 })
      .pause(120)
      .move({ origin: grid })
      .pause(120)
      .release());
    await gu.waitForServer();
    // The row leaves the unassigned panel and becomes an event on/near the dropped day.
    await driver.wait(async () => !(await getUnassignedTitles()).includes("No Date Yet"), 2000);
    const ev = await getEventByTitle("No Date Yet");
    assert.isNotNull(ev);
  ...
  const getUnassignedTitles = () =>
    driver.executeScript<string[]>("return window.gristCalendarView.getUnassignedTitles()");

  const getSelectedRecordId = () =>
    driver.executeScript<number | null>("return window.gristCalendarView.getSelectedRecordId()");

  // Looks up a Table1 rowId by its Label value, via the data engine.
  async function rowIdByLabel(label: string): Promise<number> {
    return driver.executeScript<number>(`
      const t = window.gristDocPageModel.gristDoc.get().docData.getTable("Table1");
      return t.getRowIds().find(id => t.getValue(id, "Label") === arguments[0]);
    `, label);
  }
  ...
  // The `name` of the currently-focused form field (e.g. "title", "start"), or "" if the active
  // element isn't a named field. Used to assert popup autofocus without reaching into the view.
  const activeFieldName = () =>
    driver.executeScript<string>("return (document.activeElement && document.activeElement.name) || ''");
  ...
  // A mistimed grid drag can leave TUI's create/edit form popup open; its overlay then intercepts
  // clicks in later tests. Close it (via its close button, falling back to Escape) if present.
  async function dismissCalendarPopup() {
    if (!await driver.find(".toastui-calendar-popup-overlay").isPresent().catch(() => false)) { return; }
    // Use findElements (which returns [] rather than throwing) since some popup variants have no
    // close button; fall back to Escape when it's absent.
...
describe("CalendarView setup modal", function() {
  ...
  it("creates a new column when asked, and maps it", async function() {
    ...
    const info = await driver.executeScript<{ startId: string; type: string; docTz: string }>(`
      const gd = window.gristCalendarView._view.gristDoc;
      const m = window.gristCalendarView._view.viewSection.mappedColumns();
      const startId = m && m.startDate;
      const col = window.gristCalendarView._view.viewSection.columns.peek()
        .find(c => c.colId.peek() === startId);
      return { startId, type: col && col.type.peek(), docTz: gd.docInfo.timezone.peek() };
    `);
...
describe("CalendarView drop geometry", function() {
  ...
  it("maps week-view columns to the right day (ignoring the hour gutter)", async function() {
    ...
    const info = await driver.executeScript<{ centers: Array<{ x: number; y: number }>; start: string }>(`
  ...
  it("maps month-view cells to the right day (ignoring the day-name header)", async function() {
    await driver.find(".test-calendar-perspective-month").click();
    const info = await driver.executeScript<{ cells: Array<{ x: number; y: number }>; start: string }>(`
      const cal = window.gristCalendarView._view._calendar;
